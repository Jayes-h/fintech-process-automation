 ...
 */
async function processMyntraMacros(fileBuffers, skuData, stateConfigData, brandName, date, withInventory = true) {
  ...
  if (stateConfigData && Array.isArray(stateConfigData)) {
    for (const item of stateConfigData) {
      const stateName = normalizeStateName(item.States || item.states || item.state || item.ship_to_state);
      if (stateName) {
        stateConfigMap[stateName] = {
          tallyLedger: item['Amazon pay Ledger'] || item['amazon_pay_ledger'] || item['Debtor Ledger'] || item['debtor_ledger'] || '',
  ...
  for (const row of packedData) {
    ...
    shippedData.push({
      ...
      'Base Value': safeNumber(row.base_value || row.base_value || row.Base_Value || 0),
  ...
  for (const row of rtData) {
    ...
    returnsData.push({
      'Month': monthNumber,
      'Qty': safeNumber(row.quantity || row.qty || row.Qty || 0),
      'Base Value': safeNumber(row.base_value || row.base_amount || row.Base_Value || 0),
      'Tax IGST': safeNumber(row.igst_amt || row.igst_amount || row.IGST_Amount || row['Tax IGST'] || 0),
      'CGST Tax': safeNumber(row.cgst_amt || row.cgst_amount || row.CGST_Amount || row['CGST Tax'] || 0),
      'SGST Tax': safeNumber(row.sgst_amt || row.sgst_amount || row.SGST_Amount || row['SGST Tax'] || 0)
    });
...
const netPivotData = [
  ...
  XLSX.utils.book_append_sheet(outputWorkbook, rtoPivotSheet, 'RTO Pivot');
    ...
    XLSX.utils.book_append_sheet(
      ...
      XLSX.utils.json_to_sheet(
        stateConfigData.map(item => ({
          ...
          'Tally Ledger': item['Amazon Pay Ledger'] || item['tally_ledger'] || item['Debtor Ledger'] || '',
