 ...
 * GET /api/flipkart-state-config/:brandId/:sellerPortalId
 */
 ...
 * GET /api/flipkart-state-config/brand/:brandId
 */
exports.getStateConfigsByBrand = async (req, res, next) => {
  try {
    const { brandId } = req.params;

    const stateConfigs = await FlipkartStateConfig.findAll({
      where: { brandId },
      include: [
        { model: Brands, as: 'brand', attributes: ['id', 'name'] },
        { model: SellerPortals, as: 'sellerPortal', attributes: ['id', 'name'] }
 ...
 * POST /api/flipkart-state-config
 */
exports.createOrUpdateStateConfig = async (req, res, next) => {
  try {
    ...
    const [stateConfig, created] = await FlipkartStateConfig.findOrCreate({
      where: { brandId, sellerPortalId },
      defaults: {
        brandId,
        sellerPortalId,
    ...
    res.json({
      success: true,
      data: stateConfig,
      message: created ? 'Flipkart state config created successfully' : 'Flipkart state config updated successfully'
    });
  } catch (error) {
    next(error);
  }
 ...
 * PUT /api/flipkart-state-config/:id
 */
exports.updateStateConfig = async (req, res, next) => {
  try {
    ...
    const stateConfig = await FlipkartStateConfig.findByPk(id);
    ...
    res.json({
      success: true,
      data: stateConfig,
      message: 'Flipkart state config updated successfully'
    });
  } catch (error) {
    next(error);
  }
 ...
 * DELETE /api/flipkart-state-config/:id
 */
exports.deleteStateConfig = async (req, res, next) => {
  try {
    ...
    res.json({
      success: true,
      message: 'Flipkart state config deleted successfully'
    });
  } catch (error) {
    next(error);
  }
 ...
 * POST /api/flipkart-state-config/upload/:brandId/:sellerPortalId
 * Expected Excel headers: States, Flipkart Pay Ledger, Invoice No.
 */
exports.uploadStateConfigFile = async (req, res, next) => {
  try {
    ...
    // Expected headers for Flipkart
    const expectedHeaders = ['States', 'Flipkart Pay Ledger', 'Invoice No.'];
    ...
    res.json({
      success: true,
      data: stateConfig,
      message: created ? 'Flipkart state config created successfully from file' : 'Flipkart state config updated successfully from file',
